...
int ::IntegratedHand::run(int argc, char* argv[])
{
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "HandDetectionProxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy HandDetectionProxy\n";
		}
		handdetection_proxy = HandDetectionPrx::uncheckedCast( communicator()->stringToProxy( proxy ) );
	}
